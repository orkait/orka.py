...
Consumer::
    ...
    import orka.integrations.hf_quantizer            # registers the "orka" method
...
are not yet supported on this path - pack without them, or use ``orka.integrations.hf.load_orka_model``
    ...
    for tm in manifest["tensors"]:
        ...
        if gs != KERNEL_GROUP_SIZE or bs != KERNEL_BLOCK_SIZE:
            raise ValueError(
                ...
                "Re-pack with those, or use orka.integrations.hf.load_orka_model for dense reconstruct."
            )
...
# Register on import so `import orka.integrations.hf_quantizer` is all a consumer needs.
